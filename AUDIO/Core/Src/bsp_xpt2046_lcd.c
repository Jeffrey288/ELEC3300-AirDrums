...
/******************************* ���� XPT2046 ȫ�ֱ��� ***************************/
//Ĭ�ϴ�����������ͬ����Ļ���в��죬�����µ��ô���У׼������ȡ
strType_XPT2046_TouchPara strXPT2046_TouchPara[] = { -0.006464, -0.073259,
		280.358032, 0.074878, 0.002052, -6.545977, //ɨ�跽ʽ0
...
static uint8_t XPT2046_ReadAdc_Smooth_XY(
	...
	do {
		...
		ucCount++;
			...
			&& (ucCount < 10)); //�û����������ʱ��TP_INT_IN�ź�Ϊ�� ���� ucCount<10
...
uint8_t XPT2046_Touch_Calibrate(uint8_t LCD_Mode) {
	...
	for (i = 0; i < 4; i++) {
		...
		//����ո񣬾�����ʾ
		sprintf(cStr, "%*c%s",
				((LCD_X_LENGTH / (((sFONT*) LCD_GetFont())->Width))
						- strlen(pStr)) / 2, ' ', pStr);
		ILI9341_DispStringLine_EN(LCD_Y_LENGTH >> 1, cStr);

		//����ո񣬾�����ʾ
		sprintf(cStr, "%*c%d",
				((LCD_X_LENGTH / (((sFONT*) LCD_GetFont())->Width)) - 1) / 2,
				' ', i + 1);
		ILI9341_DispStringLine_EN(
				(LCD_Y_LENGTH >> 1) - (((sFONT*) LCD_GetFont())->Height), cStr);
	...
	usTest_x = ((CalibrationFactor.An * strScreenSample[3].x)
			...
			+ CalibrationFactor.Cn) / CalibrationFactor.Divider;//ȡһ�������Xֵ
	usTest_y = ((CalibrationFactor.Dn * strScreenSample[3].x)
			+ (CalibrationFactor.En * strScreenSample[3].y)
			+ CalibrationFactor.Fn) / CalibrationFactor.Divider;   //ȡһ�������Yֵ

	usGap_x =
			(usTest_x > strCrossCoordinate[3].x) ?
					(usTest_x - strCrossCoordinate[3].x) :
					(strCrossCoordinate[3].x - usTest_x); //ʵ��X�������������ľ��Բ�
	usGap_y =
			(usTest_y > strCrossCoordinate[3].y) ?
					(usTest_y - strCrossCoordinate[3].y) :
					(strCrossCoordinate[3].y - usTest_y); //ʵ��Y�������������ľ��Բ�
	...
	/* У׼ϵ��Ϊȫ�ֱ��� */
	strXPT2046_TouchPara[LCD_Mode].dX_X = (CalibrationFactor.An * 1.0)
			/ CalibrationFactor.Divider;
	strXPT2046_TouchPara[LCD_Mode].dX_Y = (CalibrationFactor.Bn * 1.0)
			/ CalibrationFactor.Divider;
	strXPT2046_TouchPara[LCD_Mode].dX = (CalibrationFactor.Cn * 1.0)
			/ CalibrationFactor.Divider;

	strXPT2046_TouchPara[LCD_Mode].dY_X = (CalibrationFactor.Dn * 1.0)
			/ CalibrationFactor.Divider;
	strXPT2046_TouchPara[LCD_Mode].dY_Y = (CalibrationFactor.En * 1.0)
			/ CalibrationFactor.Divider;
	strXPT2046_TouchPara[LCD_Mode].dY = (CalibrationFactor.Fn * 1.0)
			/ CalibrationFactor.Divider;
	...
	//����ո񣬾�����ʾ
	sprintf(cStr, "%*c%s",
			((LCD_X_LENGTH / (((sFONT*) LCD_GetFont())->Width)) - strlen(pStr))
					/ 2, ' ', pStr);
	ILI9341_DispStringLine_EN(LCD_Y_LENGTH >> 1, cStr);
...
void imagebuilder(int posX, int posY, int resX, int resY,
		const unsigned short bitmap[]) {
	int stepcounter = 0;
	int totalsteps = resX * resY;
	LCD_OpenWindow(posX, posY, resX, resY);
...
void volumecontrol(uint8_t data, uint8_t limit) {

	LCD_SetBackColor(WHITE);
	ILI9341_Clear(70, 70, 100, 100);

	LCD_SetTextColor(RED);
	ILI9341_DrawCircle(100, 100, (data / limit) * 5, 1);
...
int StartProcess() {
	// Waiting starting signal e.g SD Card..
...
int Interface(int type, int xpos, int ypos) {
	char xposition[3];
...
//		LCD_DrawString(xpos,20,"XPosition");
		break;

	case 1:
//		LCD_Clear(0,0,320,240,WHITE);
//		sprintf(xposition, "%03d", ypos);
//		LCD_DrawString(40, ypos,"XPosition");
		break;
...
void TouchReturn() {
	...
	int currentfile = FileSelector(TouchXPOS, TouchYPOS, 0, 4);
	char xposition[1];
	sprintf(xposition, "%01d", currentfile);
	LCD_DrawString(40, 20, "Current");
	LCD_DrawString(90, 20, xposition);

}

void XPT2046_Touch(int posinfo[2]) {
...
//			cinfo.pre_x = cinfo.x;
//			cinfo.pre_y = cinfo.y;
//			//imagebuilder(0,0,276,244,shit);
//			//imagebuilder(0,0,240,320,shittest);
//			//volumecontrol(cinfo.x,320);
//			//PLOTTriangle(200,200, 10,cinfo.x,320);
//
//			// Display position data
//			char xposition[3];
//			sprintf(xposition, "%03d",cinfo.x);
//			LCD_DrawString(40,20,"XPosition");
//			LCD_DrawString(120,20,xposition);
//			char yposition[3];
//			sprintf(yposition, "%03d",cinfo.y);
//			LCD_DrawString(40,40,"YPosition");
//			LCD_DrawString(120,40,yposition);
//
//
//
//
//
//			if((cinfo.x > 200)&&(cinfo.y<100))
//			{
//				Interface(0,cinfo.x,-1);
//				HAL_GPIO_WritePin(GPIOB,GPIO_PIN_5,0);
//				HAL_GPIO_WritePin(GPIOB,GPIO_PIN_0,1);
//				HAL_GPIO_WritePin(GPIOB,GPIO_PIN_1,1);
//			}
//			else if((cinfo.x > 200)&&(cinfo.y>100))
//			{
//				Interface(1,-1,cinfo.y);
